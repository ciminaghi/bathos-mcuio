...
void nrf51_uart_irq_handler(void *_priv)
{
	struct nrf51_uart_priv *priv = _priv;
	...
	for (i = 0; readl(base + UART_EVENT_RXDRDY); ) {
		 ...
		 */
		(void)bathos_dev_push_chars(priv->dev, c, i);
		i = 0;
	}
	if (i)
		(void)bathos_dev_push_chars(priv->dev, c, i);
...
int nrf51_uart_init(struct nrf51_uart_priv *priv)
{
	const struct nrf51_uart_platform_data *plat = priv->platform_data;
	uint32_t base = priv->platform_data->base;
...
static int nrf51_uart_open(struct bathos_pipe *pipe)
{
	...
	stat = nrf51_uart_init(pipe->dev->priv);
	if (stat)
		return stat;
