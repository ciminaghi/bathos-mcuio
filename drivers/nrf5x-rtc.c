 ...
 */
/* nrf5x family rtc driver, just counts jiffies */
...
int nrf5x_rtc_init(const struct nrf5x_rtc_platform_data *plat)
{
	uint32_t prescaler = (CONFIG_LFCLK / CONFIG_HZ) - 1;
...
void nrf5x_irq_handler(struct nrf5x_rtc_platform_data *plat)
{
	uint32_t b = (uint32_t)plat->base;
