...
static void input_pipe_ready_handle(struct event_handler_data *ed)
{
	...
	struct rb_master_private_data *priv = my_pipe->dev->priv;
	const struct rb_master_platform_data *plat =
		my_pipe->dev->platform_data;
	int stat, l = plat->packet_size - priv->input_bytes;

	if (priv->packet_in_ipipe_buffer && !priv->filter_out_packet)
		pr_debug("%s: input overrun\n", __func__);
	stat = pipe_read(pipe, &priv->ipipe_buffer[priv->input_bytes], l);
	if (stat < 0) {
		pr_debug("%s: error in pipe_read\n", __func__);
		return;
	}
	priv->input_bytes += stat;
	if (priv->input_bytes < plat->packet_size)
		return;
	if (plat->filter && plat->filter(priv->ipipe_buffer, plat->packet_size,
					 plat->filter_data))
