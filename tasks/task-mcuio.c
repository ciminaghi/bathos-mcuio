			      ...
			      int index, struct mcuio_range *out)
{
	...
	if (check_data_pointer(f) < 0)
		return -1;
	if (check_data_pointer(out) < 0)
		return -1;
	r = &f->ranges[index];
	if (check_pgm_pointer(r))
		return -1;
...
{
	if (check_data_pointer(r))
		return NULL;
	if (check_data_pointer(out))
		return NULL;
	if (!r->ops)
		return NULL;
	if (is_data_pointer(r->ops)) {
		*out = *r->ops;
		return out;
	}
